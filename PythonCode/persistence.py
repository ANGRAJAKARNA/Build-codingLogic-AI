...
# Default file path for progress data (single global file — kept only as
# the fallback default for callers that don't pass a per-visitor path)
PROGRESS_FILE = Path(__file__).parent / "user_progress.json"

# Root directory under which each visitor gets their own subdirectory of
# JSON files, so concurrent visitors on Streamlit Cloud's one shared
# process don't read/write each other's progress/history.
VISITORS_ROOT = Path(__file__).parent / "data" / "visitors"


def get_visitor_dir(visitor_id: str) -> Path:
    """
    Return (creating if needed) the per-visitor data directory for the
    given visitor_id. Sanitizes the id defensively even though it's
    generated by this app itself (visitor_identity.py) — cheap insurance
    against it ever ending up as a path-traversal-shaped value.
    """
    safe_id = re.sub(r'[^a-zA-Z0-9_-]', '', str(visitor_id)) or "unknown"
    visitor_dir = VISITORS_ROOT / safe_id
...
def atomic_write_json(path: Path, data: Dict) -> None:
    ...
    os.replace() over the real path. os.replace() is atomic on both POSIX
    and Windows, so a crash or a concurrent write from another tab never
    leaves a half-written/corrupt file — the readable file is always either
    the old complete version or the new complete version, never a mix.
    """
    path = Path(path)
...
# The only keys in `progress` that hold the {completed, skipped, times} shape:
# the three core Python difficulty stages, plus the automation question
# categories from automation_questions.py (Selenium/Robot Framework/pytest).
# Kept as an explicit, stable set rather than derived from questions.py at
# import time, so persistence.py stays importable/testable without a hard
# dependency on the question bank. Everything else bolted onto the progress
# dict (streak, unlocked_achievements, solved_without_hints, comeback,
# night_solve, early_solve, ...) must pass through save/load unmodified
...
def _serialize_progress(progress: Dict) -> Dict:
    ...
    Sets are converted to sorted lists for the three stage keys;
    every other top-level key (streak, achievements, etc.) is passed
    through unchanged.
    """
    serialized = {}
...
def _deserialize_progress(data: Dict) -> Dict:
    ...
    Lists are converted to sets for completed/skipped on the three stage
    keys; every other top-level key is passed through unchanged.
    """
    progress = {}
    for key, stage_data in data.items():
        if key in STAGE_KEYS:
            progress[key] = {
...
def save_progress(progress: Dict, file_path: Optional[Path] = None) -> bool:
    ...
    try:
        ...
        # Write atomically so a concurrent read (or a crash mid-write) never
...
def ensure_all_stages(progress: Dict) -> Dict:
    """
    Backfill any stage keys missing from a loaded progress dict with empty
    defaults, in place. Needed for progress saved before a new stage
    existed (e.g. the automation categories added after some visitors
    already had progress files) — without this, looking up a stage that
    predates it would KeyError instead of just showing 0 solved.
    """
...
def get_default_progress() -> Dict:
    """
    Get a fresh default progress structure with sets, covering every
    trackable stage (core Python difficulties + automation categories).
    """
    return {stage: {"completed": set(), "skipped": set(), "times": {}} for stage in STAGE_KEYS}
...
def get_stats(progress: Dict) -> Dict[str, Any]:
    ...
    # Get actual question counts dynamically from questions module (includes
    # both the core Python stages and the automation categories)
    try:
        from questions import get_all_questions
        question_counts = {stage: len(questions) for stage, questions in get_all_questions().items()}
    except ImportError:
        # Fallback to defaults if questions module not available
        question_counts = {"Basic": 60, "Intermediate": 50, "Advanced": 40}

    for stage, data in progress.items():
        if stage not in STAGE_KEYS:
            # Skip non-stage keys bolted onto progress (streak, achievements, etc.)
            continue
...
def import_progress(encoded: str, merge_with_existing: bool = False,
                     existing_progress: Optional[Dict] = None) -> Optional[Dict]:
    """
    Import progress from base64 encoded string.

    Args:
        encoded: Base64 encoded progress string
        merge_with_existing: If True, merge with existing_progress (keep best times)
        existing_progress: The caller's current in-memory progress dict to merge
            into. Callers should pass this explicitly (e.g. st.session_state.progress)
            rather than relying on a default disk path, since with per-visitor
            storage there is no single global file to fall back to.
    ...
    try:
        ...
        if merge_with_existing and existing_progress:
            imported = merge_progress(existing_progress, imported)
